...
```{r}
metBR <- getMetrics(test.BR)
metMCR <- getMetrics(test.MCR)
...
```{r}
resBR <- classifyRain(test.BR, t.values = trBR, threshold = "min")
resMCR <- classifyRain(test.MCR, t.values = trMCR, threshold = "min")
